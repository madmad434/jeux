...
Dix jeux HTML et dix éditeurs XML, lancés depuis un menu unique.
...
Quizz, Jeu de Mot, Anagrammes, Mots masqués, Décodage, et les dix éditeurs XML.
Le Jeu des paires, la Mémoire et la Mémoire calculée n'utilisent aucun fichier
externe : leurs cartes sont intégrées.
...
1. Dépose le fichier dans le dossier concerné (`JeuPuzzle/Images`,
   `JeuQuizz/Questionnaires`, `JeuDeMot/FichiersMots`, `JeuAnagramme/Mots`,
   `JeuMotsMasques/Textes`, `JeuDecode/Textecode`).
2. Ajoute son chemin dans `assets/js/ressources.js` — c'est une simple liste,
   éditable directement sur GitHub :
   ```js
   "puzzle": [
     "JeuPuzzle/Images/image.jpeg",
...
│   └── favicons/
│       ├── jeux.webp             partagée par 4 jeux + les éditeurs
│       ├── jeudemot.png
│       ├── quizz.webp
│       ├── paires.png
│       ├── sequence.png
...
├── JeuDesPaires/JeuDesPaires.html    retrouver les paires de cartes
├── JeuMemoire/JeuMemoire.html        restituer une séquence (couleurs,
│                                     lettres, chiffres ou solfège)
├── JeuCalculs/JeuCalculs.html        associer un calcul à son résultat
├── JeuQuizz/
│   ├── JeuQuizz.html
...
└── JeuDecode/
    ├── JeuDecodage.html
    └── Textecode/                MessageCodes.xml + 2 éditeurs
```

## Sauvegarder et reprendre une partie

Les **dix jeux** partagent le même mécanisme, servi par `assets/js/partie.js` :
bouton **💾 Sauver** pendant la partie, bouton **Reprendre** (ou **Charger
partie**) sur l'écran de départ. Le fichier `.json` est écrit directement, sous
un nom horodaté, et une fenêtre indique lequel.

La sauvegarde embarque le contenu de la partie — mots, phrases, questions,
séquence tirée au sort. Une partie commencée sur un poste se termine donc sur
un autre, même si le fichier XML d'origine n'y est pas installé.

La reprise repart au début de l'élément en cours (question, mot, phrase,
séquence) : le score, l'historique et le chronomètre sont restaurés, mais pas
la saisie en cours, qui n'aurait pas de sens une fois la grille reconstruite.
...
La fenêtre est la même partout : panneau ancré à gauche, une croix en haut à
droite, un pied avec **🖨 Imprimer** et **Fermer**, <kbd>Échap</kbd> pour
refermer. Le bouton *Imprimer* recopie le texte de l'aide seul, en noir sur
blanc, sans le jeu autour ni les boutons.

L'aide du menu résume les dix jeux et les dix éditeurs, dit quel jeu accepte
quel contenu et dans quel dossier, et explique la différence entre l'ouverture
locale et l'ouverture depuis un serveur. Tout ce qui relève de la publication
et de l'inventaire est regroupé dans une **section repliable « Partie
technique »**, fermée par défaut : repliée, elle n'apparaît pas non plus à
l'impression.
...
- Touches **1** à **9** et **0** pour lancer un jeu directement.
- Les dix éditeurs sont regroupés derrière le bandeau **« Préparer le contenu
...
de fichier, le Jeu des paires non, le Puzzle téléchargeait directement sans rien
...
Le bouton est sur **fond rouge** dans les dix jeux et les dix éditeurs.
...
Les **dix éditeurs XML** suivent le même déroulé, avec l'attribut
...
- les dix jeux donnent accès à leur aide **dès leur écran d'accueil**. Le
